...
Functions that interact with web sites are challenging to work with and troubleshoot. This is largely because this is a complex action and  problems are difficult to isolate. Workplaces may block R from interacting with web sites. A user's internet or the target web site may be down or be slow to respond. A recent change to the website may have been incompatible with the code.
...
Some NOAA stations also collect meteorological data. If available (check with `noaa.parameters(stn = "8461490")`), these data can be downloaded in the `noaa()` call by setting `met = TRUE`.
